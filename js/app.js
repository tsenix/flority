(() => {
    ...
    function initSliders() {
        if (document.querySelector(".products__body-slider")) new Swiper(".products__body-slider", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            autoHeight: true,
    ...
    if (modalOpenButtons.length) {
        ...
        const modalForm = document.querySelector(".popup__content");
        ...
        function closeModal(confirm) {
            ...
            if (confirm) console.log("submit");
        }
        modal.addEventListener("click", (e => {
            if (e.target === modalWrap) closeModal(false);
        }));
        document.addEventListener("keydown", (e => {
            if (e.key === "Escape" && modal.classList.contains("popup_show")) closeModal(false);
        }));
        modalForm.addEventListener("submit", (e => {
            ...
            const quantityInput = modalForm.querySelector("[data-quantity-value]");
        ...
        function validateFields() {
            ...
            inputs.forEach((input => {
                if (!input.checkValidity()) input.classList.add("error"); else input.classList.remove("error");
            }));
        }
        ...
        allInputs.forEach((input => {
            input.addEventListener("input", (() => {
                console.log("input");
                if (input.checkValidity()) input.classList.remove("error");
            }));
        }));
    }
    const productGallery = document.querySelector(".product__gallery");
    if (productGallery) {
        const productImage = productGallery.querySelector(".product__main-image img");
        const productSliders = productGallery.querySelectorAll(".product__image img");
        for (const slide of productSliders) slide.addEventListener("click", (() => {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        if (productGallery) {
            const productImage = productGallery.querySelector(".product__main-image img");
            const productSliders = productGallery.querySelectorAll(".product__image img");
            productImage.src = productSliders[0].src;
        }
    }));
    const items = document.querySelectorAll(".feedback__form-rate-item");
    if (items.length) {
        let selectedIndex = -1;
        items.forEach(((item, index) => {
            ...
            item.addEventListener("mouseenter", (() => {
                items.forEach(((el, i) => {
                    el.classList.toggle("hover", i <= index);
                }));
            }));
            item.addEventListener("click", (() => {
                selectedIndex = index;
                items.forEach(((el, i) => {
                    el.classList.toggle("checked", i <= selectedIndex);
                }));
            }));
        }));
        const container = document.querySelector(".feedback__form-rate");
